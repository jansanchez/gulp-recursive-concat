...
function RecursiveConcat(options) {
	...
	this.stream._transform = function(chunk, encoding, callback){
        ...
        console.log("chunk relative",chunk.relative);
...
RecursiveConcat.prototype.walk = function(file){
	...
	var newFile = utils.getFolderName(file.base, file.newDirname),
		...
		numberDirectories = 0,
        walking = true,
        ignored = this.options.ignore && this.captureIgnore(this.cleanDirname(file.newDirname));
	...
	if (this.walkCounter === numberFiles) {
		this.walkCounter = 0;
		this.newFileGenerated = this.makeNewFile(newFile, file, ignored);
...
RecursiveConcat.prototype.makeNewFile = function(nameFile, file, ignored){
	...
	if (arrayBasePath !== null) {
		if(this.options.outside && !ignored) {
			arrayBasePath.pop();
		}
